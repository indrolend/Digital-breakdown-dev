...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; try { $url='%CONTROL_URL%?t=' + [DateTimeOffset]::UtcNow.ToUnixTimeSeconds(); Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile '%CONTROL_TMP%' -TimeoutSec 30; $text=Get-Content '%CONTROL_TMP%' -Raw; if ($text -notmatch 'DIGITAL BREAKDOWN DEV CONTROL') { throw 'Downloaded control script failed validation.' }; $text=$text.Replace('$uri = \"$ManifestUrl?t=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())\"','$uri = \"${ManifestUrl}?t=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())\"'); [IO.File]::WriteAllText('%CONTROL_TMP%',$text,(New-Object Text.UTF8Encoding($false))); Move-Item -Force '%CONTROL_TMP%' '%CONTROL_PS1%'; exit 0 } catch { Write-Host $_.Exception.Message -ForegroundColor Red; Remove-Item '%CONTROL_TMP%' -Force -ErrorAction SilentlyContinue; exit 1 }"
if errorlevel 1 (
  echo.
