...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "try { Invoke-WebRequest -UseBasicParsing '%CONTROL_URL%?t=' + [DateTimeOffset]::UtcNow.ToUnixTimeSeconds() -OutFile '%CONTROL_PS1%'; exit 0 } catch { Write-Host $_.Exception.Message -ForegroundColor Red; exit 1 }"
if errorlevel 1 (
  echo.
